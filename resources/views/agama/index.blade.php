...
@section('button')
    <li class="nav-item mr-2">
        <a href="{{ url('/agama/calendar') }}" class="btn btn-success nav-link" style="color: white">Calendar</a>
    </li>
    <li class="nav-item mr-2">
        <a href="{{ url('/agama/tambah') }}" class="btn btn-primary nav-link" style="color: white">+ Tambah</a>
    </li>
@endsection
@section('isi')
    <form action="{{ url('/agama') }}" class="mr-2 ml-2">
                    ...
                    <tbody>
                        ...
                        @else
                            @foreach ($agamas as $key => $agama)
                                <tr>
                                    <td class="text-center" style="position: sticky; left: 0; background-color: rgb(235, 235, 235); z-index: 1; vertical-align: middle;">{{ ($agamas->currentpage() - 1) * $agamas->perpage() + $key + 1 }}.</td>
                                    <td class="text-center" style="vertical-align: middle;">{{ $agama->type ?? '-' }}</td>
                                    <td class="text-center" style="vertical-align: middle;">{{ $agama->day ?? '-' }}</td>
                                    <td class="text-center" style="vertical-align: middle;">{{ $agama->date ?? '-' }}</td>
                                    <td class="text-center" style="vertical-align: middle;">{{ $agama->title ?? '-' }}</td>
                                    <td class="text-center" style="vertical-align: middle;">{!! $agama->note ? nl2br(e($agama->note)) : '-' !!}</td>
                                    <td class="text-center" style="vertical-align: middle;">{{ $agama->contact_person ?? '-' }}</td>
                                    <td class="text-center" style="vertical-align: middle;">{{ $agama->createdBy->name ?? '-' }}</td>
                                    <td class="text-center" style="vertical-align: middle;">{{ $agama->updatedBy->name ?? '-' }}</td>
                                    <td style="vertical-align: middle;">
                                        <div style="display: flex; gap: 5px;">
                                            <a href="{{ url('/agama/edit/'.$agama->id) }}" class="btn btn-primary btn-sm" title="Edit agama"><i class="fa fa-edit"></i></a>

                                            <form action="{{ url('/agama/delete/'.$agama->id) }}" method="post">
                                                @method('delete')
                                                @csrf
                                                <button title="Delete agama" class="border-0 btn btn-danger btn-sm" onClick="return confirm('Are You Sure')"><i class="fa fa-trash"></i></button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
