                ...
                var calendar = new FullCalendar.Calendar(calendarEl, {
                    ...
                    events: [
                        @foreach($olahragas as $olahraga)
                            @if($olahraga->type == 'Jadwal Tetap')
                            {
                                title: '{{ $olahraga->title }}',
                                daysOfWeek: [{{ $dayMap[$olahraga->day] ?? 0 }}],
                                allDay: true,
                                extendedProps: {
                                    note: `{!! nl2br(e($olahraga->note)) !!}`,
                                    contact_person: '{{ $olahraga->contact_person }}'
                                }
                            },
                            @elseif($olahraga->type == 'Event')
                            {
                                title: '{{ $olahraga->title }}',
                                start: '{{ \Carbon\Carbon::parse($olahraga->date)->format("Y-m-d") }}',
                                allDay: true,
