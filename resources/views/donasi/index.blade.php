...
@section('button')
    <li class="nav-item mr-2">
        <a href="{{ url('/donasi/export') }}{{ $_GET?'?'.$_SERVER['QUERY_STRING']: '' }}" class="btn btn-success nav-link" style="color: white"><i class="far fa-file-excel mr-1"></i>Export</a>
    </li>
@endsection
@section('isi')
    <div class="d-none d-md-block">
        <form action="{{ url('/donasi') }}" class="mr-2 ml-2">
            ...
            <div class="table-responsive" style="border-radius: 10px">
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        @else
                            @foreach ($transaction_donasi as $key => $donasi)
                                <tr>
                                    <td class="text-center" style="position: sticky; left: 0; background-color: rgb(235, 235, 235); z-index: 1; vertical-align: middle;">{{ ($transaction_donasi->currentpage() - 1) * $transaction_donasi->perpage() + $key + 1 }}.</td>
                                    <td class="text-center" style="vertical-align: middle;">{{ $donasi->user->alamat ?? '-' }}</td>
                                    <td class="text-center" style="vertical-align: middle;">{{ $donasi->user->name ?? '-' }}</td>
                                    <td class="text-center" style="vertical-align: middle;">{{ $donasi->date ?? '-' }}</td>
                                    <td class="text-center" style="vertical-align: middle;">{{ $donasi->type ?? '-' }}</td>
                                    <td class="text-center" style="vertical-align: middle;">{{ $donasi->payment_source ?? '-' }}</td>
                                    <td class="text-center" style="vertical-align: middle;">Rp {{ number_format($donasi->nominal) }}</td>
                                    ...
                                    <td class="text-center" style="vertical-align: middle;">
                                        <div style="display: flex; gap: 5px;">
                                            <a href="{{ url('/donasi/show/'.$donasi->id) }}" class="btn btn-info btn-sm" title="show donasi"><i class="fa fa-eye"></i></a>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
